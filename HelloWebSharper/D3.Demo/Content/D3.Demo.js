...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,D3,Demo,D3Helpers,d3,Concurrency,Exception,ZoomableMap,document,console,PrintfHelpers,Number;
 Runtime.Define(Global,{
  D3:{
   Demo:{
    D3Helpers:{
     "D3.Json.Static":function(url)
     {
      return D3Helpers.Defer(function(callback)
      {
       return d3.json(url,Runtime.CreateFuncWithArgs(callback));
      });
     },
     "D3.Tsv.Static":function(url)
     {
      return D3Helpers.Defer(function(callback)
      {
       return d3.tsv(url,Runtime.CreateFuncWithArgs(callback));
      });
     },
     Defer:function(job)
     {
      return Concurrency.FromContinuations(function(tupledArg)
      {
       ...
       error=tupledArg[1];
       tupledArg[2];
       return job(function(tupledArg1)
       {
        var err,data;
        err=tupledArg1[0];
        data=tupledArg1[1];
        return err?error(Exception.New1("Defer: failed")):ok(data);
       });
      });
     }
    },
    ZoomableMap:{
     ...
     {
      return ZoomableMap.Render(document.getElementById("main"));
     }),
     Render:function(canvas)
     {
      var dimensions,currentStateIndex,resetCurrentState,projection,objectArg,path,svg,onClicked,value2,svgGroup,arg00;
      dimensions={
       Width:960,
       Height:500
      };
      ...
      {
       currentStateIndex[0]=-1;
       return;
      };
      projection=d3.geo.albersUsa().scale(1070).translate([dimensions.Width/2,dimensions.Height/2]);
      objectArg=d3.geo.path();
      path=objectArg.projection(projection);
      svg=d3.select(canvas).append("svg").attr("width",dimensions.Width).attr("height",dimensions.Height);
      onClicked=function(eventData)
      {
       var x,y,k,stateClickedIndex,stateClicked,clo1,clo11,_1,patternInput,cY,cX,svgG,value,value1;
       x=0;
       y=0;
       k=0;
       stateClickedIndex=eventData[1];
       stateClicked=eventData[0];
       clo1=function(_)
       {
        var s;
        s="stateClickedIndex: "+Global.String(_);
        return console?console.log(s):undefined;
       };
       clo1(stateClickedIndex);
       clo11=function(_)
       {
        var s;
        s="currentStateIndex: "+Global.String(_);
        return console?console.log(s):undefined;
       };
       clo11(stateClickedIndex);
       if((PrintfHelpers.prettyPrint(stateClicked)!=="undefined"?stateClickedIndex!==-1:false)?currentStateIndex[0]!==stateClickedIndex:false)
        {
         patternInput=path.centroid(stateClicked);
         cY=patternInput[1];
         cX=patternInput[0];
         x=Number(cX);
         y=Number(cY);
         k=4;
         _1=void(currentStateIndex[0]=stateClickedIndex);
        }
       else
        {
         ...
         k=1;
         _1=resetCurrentState(null);
        }
       svgG=d3.select("#svg-group");
       value=svgG.selectAll("path").classed("active",function(_arg1,i)
       {
        return currentStateIndex[0]!==-1?currentStateIndex[0]===i:false;
       });
       value1=svgG.transition().duration(750).attr("transform","translate("+dimensions.Width/2+","+dimensions.Height/2+")"+("scale("+k+")")+("translate("+-x+","+-y+")"));
       return;
      };
      value2=svg.append("rect").attr("class","background").attr("width",dimensions.Width).attr("height",dimensions.Height).on("click",Runtime.CreateFuncWithArgs(onClicked));
      svgGroup=svg.append("g").attr("id","svg-group");
      arg00=Concurrency.Delay(function()
      {
       var x;
       x=D3Helpers["D3.Json.Static"]("Content/us.json");
       return Concurrency.Bind(x,function(_arg1)
       {
        var states,borders,value,value1;
        states=ZoomableMap.topojson().feature(_arg1,_arg1.objects.states).features;
        borders=ZoomableMap.topojson().mesh(_arg1,_arg1.objects.states,function(tupledArg)
        {
         var a,b;
         a=tupledArg[0];
         b=tupledArg[1];
        ...
        value=svgGroup.append("g").attr("id","states").selectAll("path").data(states).enter().append("path").attr("d",path).on("click",Runtime.CreateFuncWithArgs(onClicked));
        value1=svgGroup.append("path").datum(borders).attr("id","state-borders").attr("d",path);
        return Concurrency.Return(null);
       });
      });
      return Concurrency.Start(arg00,{
     ...
     topojson:Runtime.Field(function()
     {
 ...
 Runtime.OnInit(function()
 {
  ...
  D3Helpers=Runtime.Safe(Demo.D3Helpers);
  d3=Runtime.Safe(Global.d3);
  Concurrency=Runtime.Safe(Global.WebSharper.Concurrency);
  Exception=Runtime.Safe(Global.WebSharper.Exception);
  ZoomableMap=Runtime.Safe(Demo.ZoomableMap);
  document=Runtime.Safe(Global.document);
  console=Runtime.Safe(Global.console);
  PrintfHelpers=Runtime.Safe(Global.WebSharper.PrintfHelpers);
